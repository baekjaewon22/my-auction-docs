...
const output = process.argv[3] || 'tmp-expense-classification/local-import-expense-v2.sql';
...
VALUES
  (${sql(sourceId)}, ${sql(batchId)}, ${sql(row.source_type)}, ${Number(row.row_index || 0)}, ${sql(row.source_key)}, ${sql(row.transaction_at || '')}, ${sourceAmount}, ${sql(direction)}, ${sql(row.merchant_name || '')}, ${sql(row.description || '')}, ${sql(row.card_last4 || '')}, NULL, ${sql(rawJson)})
...
VALUES
  (${sql(ledgerId)}, ${sql(reconId)}, ${sql(sourceId)}, 'expense', ${sql(String(row.transaction_at || '').slice(0, 10))}, ${sql(row.branch || '')}, ${sql(row.owner_name || '')}, ${sql(row.category || '')}, ${sql(row.item || '')}, ${ledgerAmount}, ${sql(direction)}, ${sql(row.memo || '')}, 'confirmed', 'local-import', datetime('now', '+9 hours'))
...
console.log(JSON.stringify({ output, rows: rows.length, fileHash }, null, 2));
