...
export default function Login() {
  ...
  const [savedEmail] = useState(() => {
    try {
      ...
      const emailOnly = String(saved?.email || '');
      if (saved?.password) localStorage.setItem(SAVED_CRED_KEY, JSON.stringify({ email: emailOnly }));
      return emailOnly;
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRegister) {
      ...
      if (password.length < MIN_PASSWORD_LENGTH) { setError(`비밀번호는 ${MIN_PASSWORD_LENGTH}자 이상이어야 합니다.`); return; }
    }
    ...
    try {
      if (isRegister) {
        ...
        // 아이디/비밀번호 저장
        if (rememberMe) {
          localStorage.setItem(SAVED_CRED_KEY, JSON.stringify({ email }));
        } else {
          localStorage.removeItem(SAVED_CRED_KEY);
        }
  ...
  const handleForgotSend = async () => {
    ...
    try {
      const response = await api.auth.forgotSend(forgotEmail, forgotName, forgotPhone);
  ...
  const handleForgotVerify = async () => {
    ...
    try {
      const res = await api.auth.forgotVerify(forgotChallengeId, verifyCode);
  ...
  const handleForgotReset = async () => {
    ...
    if (newPassword.length < MIN_PASSWORD_LENGTH) { setError(`비밀번호는 ${MIN_PASSWORD_LENGTH}자 이상이어야 합니다.`); return; }
    setLoading(true); setError('');
    try {
      ...
      setVerifyCode(''); setForgotChallengeId(''); setNewPassword(''); setNewPasswordConfirm('');
    } catch (err: any) { setError(err.message); }
    finally { setLoading(false); }
  };
              ...
              {forgotStep === 'reset' && (
                <>
                  <div className="form-group" style={{ marginBottom: 10 }}>
                    ...
                    <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder={`새 비밀번호 (${MIN_PASSWORD_LENGTH}자 이상)`} style={{ width: '100%' }} />
                  </div>
                  <div className="form-group" style={{ marginBottom: 14 }}>
