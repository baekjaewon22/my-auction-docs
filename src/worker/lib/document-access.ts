...
import {
  isFreelancerViewer,
  isMyAuctionDocument,
} from './template-access.ts';

type DocumentViewer = Pick<JwtPayload, 'sub' | 'role' | 'branch' | 'department' | 'login_type'>;
type DocumentAccessRecord = Pick<Document, 'id' | 'template_id' | 'is_myauction' | 'author_id' | 'branch' | 'department' | 'status'>;
...
export async function getDocumentAccessRecord(
  ...
  return db.prepare(
    'SELECT id, template_id, is_myauction, author_id, branch, department, status FROM documents WHERE id = ?'
...
export async function canReadDocument(
  ...
  prefetchedAssignedDocumentIds?: ReadonlySet<string>,
): Promise<boolean> {
  ...
  if (prefetchedAssignedDocumentIds) {
    if (prefetchedAssignedDocumentIds.has(doc.id)) return true;
  } else {
