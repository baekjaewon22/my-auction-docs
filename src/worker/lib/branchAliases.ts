...
export function branchAliases(value: unknown): string[] {
  switch (normalizeBranchName(value)) {
    case '의정부본사':
      return ['의정부본사', '의정부', '의정부지사', '본사'];
    case '서초지사':
      return ['서초지사', '서초', '강남지사', '강남'];
    case '대전지사':
      return ['대전지사', '대전'];
    case '부산지사':
      return ['부산지사', '부산'];
    case '본사관리':
      return ['본사관리', '본사 관리'];
    default:
      return compactBranchName(value) ? [compactBranchName(value)] : [];
  }
}
