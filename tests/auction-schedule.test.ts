...
test('활동별 필수 일정 정보를 검증한다', () => {
  ...
  assert.match(getAuctionScheduleValidationError('미팅', { client: '홍길동' }) || '', /미팅은/);
  assert.equal(getAuctionScheduleValidationError('미팅', { client: '홍길동', internalMeeting: true }), null);
});
...
test('화면은 월~금 5열이며 카드 텍스트를 생략하지 않는다', () => {
  ...
  assert.match(page, /\['월', '화', '수', '목', '금'\]/);
  assert.doesNotMatch(page, /프리랜서의 입찰·임장·미팅 일정을 공유합니다/);
  assert.match(css, /\.auction-schedule-week-grid[\s\S]*?grid-template-columns:\s*repeat\(5/);
  ...
  assert.match(app, /employeeViewerRoles = \['master', 'ceo', 'cc_ref', 'admin', 'manager', 'accountant', 'accountant_asst'\]/);
  assert.match(layout, /canViewAuctionSchedule[\s\S]*?'accountant'[\s\S]*?'accountant_asst'/);
